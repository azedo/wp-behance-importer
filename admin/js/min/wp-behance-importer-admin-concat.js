...
(function( $ ) {
	...
	function storeInfo() {
		if (jsonDB) {
			var fileSize		= ((jsonDB.length * 2)/1024).toFixed(2)+" kb",
					jsonFile		= JSON.parse(jsonDB),
					jsonIsDB		= (jsonIsDBt === "true") ? wpbi.string_26 : wpbi.string_27,
					jsonIsLS		= (jsonIsLSt) ? wpbi.string_26 : wpbi.string_27;
			...
			$('#storage-info').html('<h3><i class="dashicons dashicons-yes"></i> ' + wpbi.string_2 + '</h3>' +
															...
															'<ul>' +
																...
																'<li>' + wpbi.string_6 + ': ' + '<span>' + jsonFile[0].name + '</span></li>' +
																'<li>' + wpbi.string_24 + ': ' + '<span>' + jsonIsDB + '</span></li>' +
																'<li>' + wpbi.string_25 + ': ' + '<span>' + jsonIsLS + '</span></li>' +
															'</ul>' +
															'<p>' +
			...
			$('#storage-info').html('<h3><i class="dashicons dashicons-no-alt"></i>' + wpbi.string_9 + '</h3><br/><em>' + wpbi.string_10 +'</em>');
			$('#storage-info').css('display', 'block');
		}
	}
		...
		if (jsonDB) {
			...
			success: function(newData) {
				...
				if (newData.projects.length > 0) {
					...
					localStorage.setItem('wpbi_json', JSON.stringify(newResults));

					// Save JSON to DB
					$('#behanceJson input[name="wpbi_json"]').val(JSON.stringify(newResults));
					$('#behanceJson input[name="wpbi_jsonDate"]').val(jsonDate);

					var jsonResult = $('#wpbi_json_fields input[name="wpbi_json"]').val(),
							jsonDateResult = $('#wpbi_json_fields input[name="wpbi_jsonDate"]').val(),
							dataJ = {
								'action': 'wp_behance_save_json_ajax',
								'jsonToDb': jsonResult,
								'jsonDateToDb': jsonDateResult
							};
					...
					$.ajax({
						...
						error: function(errorJson) {
							alert('error');
							console.log('Couln\'t save the json on the db => '  + errorJson);
						}
					});
	...
	function doAjaxRequest(button, btnId, timestamp) {
		...
		if (jsonDB) {
			...
			localStorage.setItem('wpbi_jsonDate', jsonDate);
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (results.length > 0) {
						...
						localStorage.setItem('wpbi_json', JSON.stringify(results));

						// Save JSON to DB
						$('#behanceJson input[name="wpbi_json"]').val(JSON.stringify(results));
						$('#behanceJson input[name="wpbi_jsonDate"]').val(jsonDate);

						var jsonResult = $('#wpbi_json_fields input').serialize(),
								dataJ = {
									'action': 'wp_behance_save_json_ajax',
									'jsonToDb': jsonResult
								};
